...
if(!is_file("$mod_db")) {
	...
	$sql_entries_table = generate_sql_query("../modules/flatCal.mod/install/tpl-cals.php");
	$sql_cats_table = generate_sql_query("../modules/flatCal.mod/install/tpl-categories.php");
	$sql_prefs_table = generate_sql_query("../modules/flatCal.mod/install/tpl-preferences.php");
	...
	$sql_insert = "INSERT INTO fc_prefs ( prefs_id, prefs_status, prefs_version ) VALUES ( NULL, 'active', '$mod[version]' )";
...
if(table_exists("$mod_db","fc_prefs") < 1) {
	$prefs_version = 0;
} else {
	...
	if(!is_array($prefs)) {
		$sql_insert = "INSERT INTO fc_prefs ( prefs_id, prefs_status, prefs_version ) VALUES ( NULL, :active, :version )";
		$sth = $dbh->prepare($sql_insert);
		$sth->bindValue(':active', 'active', PDO::PARAM_STR);
		$sth->bindValue(':version', $mod['version'], PDO::PARAM_STR);
...
if($prefs_version < $mod['version']) {
	...
	$all_tables = glob("../modules/flatCal.mod/install/tpl-*.php");
